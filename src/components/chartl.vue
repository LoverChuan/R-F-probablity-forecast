<template>
  <div id="mainl" style="width: 1200px; height: 560px"></div>
</template>
...
export default {
  ...
  mounted() {
    ...
    this.$bus.$on('intervalDatal', this.demo);
    this.$bus.$on('shiftSideBar', this.reset);
    this.chart();
  },
  methods: {
    setfocus(name) {
      this.focusing = (name === 'first');
    },
    ...
    chart() {
      this.myChart = echarts.init(document.getElementById("mainl"));
      var option = {
        title: {
          text: "趋势折线图",
        },
